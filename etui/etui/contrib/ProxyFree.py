...
class ProxyMiddleware(object):
    ...
    def fetch_new_proxyes(self):
        ...
        # cur.execute("Select `SNO`,`ip` From tmpfree Where `speed`=1 and `Code`!=2 and `Time`>'%s' order by `Time` asc "%time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(time.time()-150)))
        cur.execute(
            "Select `SNO`,`ip` From tmpfree Where `speed`=1 and `Code`=3  and UNIX_TIMESTAMP(`Time`)>(UNIX_TIMESTAMP()-150) order by RAND() LIMIT 10")
        for r in cur:
            ips = "http://" + r['ip']
